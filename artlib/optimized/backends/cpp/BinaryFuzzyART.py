...
class BinaryFuzzyART(pyBinaryFuzzyART):
    """BinaryFuzzyART for Clustering. optimized with C++

    This module implements BinaryFuzzyART

    FuzzyART is a non-modular clustering model for binary input data which has been
    highly optimized for run-time performance. Fit and predict functions are
    implemented in c++ for efficient execution. This class acts as a wrapper for the
    underlying c++ functions and to provide compatibility with the artlib style and
    usage. Functionally, FuzzyART behaves as a special case of
