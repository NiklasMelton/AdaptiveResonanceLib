...
def art_model():
    ...
    lr_w = 0.1
    lr_s = 0.05
    return QuadraticNeuronART(rho=rho, s_init=s_init, lr_b=lr_b, lr_w=lr_w, lr_s=lr_s)
...
def test_category_choice(art_model):
    ...
    art_model.dim_ = 2
    i = np.array([0.2, 0.3])
    w = np.array([1.0, 0.0, 0.0, 1.0, 0.25, 0.35, 0.5])  # Mock weight (matrix, centroid, and activation parameter)
    params = {
        "rho": 0.7,
        "s_init": 0.5
    }
...
def test_update(art_model):
    ...
    assert len(updated_weight) == 7  # Check that the weight has matrix, centroid, and activation parameter
    assert updated_weight[-1] < 0.5  # Check that the activation parameter has been updated
...
def test_new_weight(art_model):
    ...
    assert len(new_weight) == 7  # Weight matrix (4 values), centroid (2 values), and activation parameter (1 value)
    assert new_weight[-1] == 0.5  # Initial activation parameter should be s_init
