...
template <typename T> struct WideMul;

// Wide multiplication type: uint32 -> uint64 (exact), uint64 -> __int128 (exact)
template <> struct WideMul<std::uint32_t> {
...
#if FRACSORT_HAS_U64
template <> struct WideMul<std::uint64_t> {
...
template <typename T> static inline void reduce_item_inplace(Item<T>& it) noexcept {
    ...
    // The binding validates that denominators are positive.
    T g = std::gcd(it.num, it.den);
    it.rnum = it.num / g;
    it.rden = it.den / g;
}

template <typename T> inline void reduce_items_inplace(Item<T>* items, size_t n) noexcept {
    for (size_t i = 0; i < n; ++i)
        reduce_item_inplace(items[i]);
}
...
static inline bool frac_greater_item(const Item<T>& a, const Item<T>& b) noexcept {
    ...
    if (a.den == b.den) {
        if (a.num != b.num)
            return a.num > b.num; // same den => compare num
...
static inline bool tie_den_desc_idx_asc(const Item<T>& a, const Item<T>& b) noexcept {
    if (a.den != b.den)
        return a.den > b.den; // larger denominator first
    return a.idx < b.idx;     // then lower index
}

template <typename T> inline void argsort_items_inplace(Item<T>* items, size_t n) {
...
template <typename T> inline size_t fracargmax_items(Item<T>* items, size_t n) {
    if (n == 0)
        return 0;
