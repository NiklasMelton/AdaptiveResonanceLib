...
def test_category_choice(art_model):
    ...
    i = np.array([0.2, 0.3])
    w = np.array(
        [0.25, 0.35, 2.0, 2.5, 0.5, 0.6, 1.2, 1.0, 5]
    )  # Mock weight vector
    art_model.W = [w]
    params = {"rho": 0.7, "alpha": 1e-5}
...
def test_update(art_model):
    ...
    assert (
        updated_weight[-1] == 6
    )  # Check that the sample count has been updated
...
def test_new_weight(art_model):
    ...
    assert (
        len(new_weight) == 8
    )  # Mean, sigma, inverse sigma, determinant, and count
